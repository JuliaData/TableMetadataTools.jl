...
to_toml(x::Union{AbstractDict, Integer, AbstractFloat,
                 Date, DateTime, Time}) = x
to_toml(x::Union{Tuple, AbstractSet}) = collect(x)
to_toml(x::AbstractArray) = vec(x)
to_toml(x::NamedTuple) = Dict(pairs(x))
...
function meta2toml(table; style::Bool=true)
    allmeta = Dict("metadata" => metadata(table, style=style),
                   "colmetadata" => colmetadata(table, style=style),
                   "style" => style)
    io = IOBuffer()
